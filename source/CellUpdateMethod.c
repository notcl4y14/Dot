...
{
	if (y == cc_front->height - 1)
	{
		return 0;
	}

	Cell* const down_cell = CellChunk_GetCell(cc_back, x, y + 1);
	CellOptions* const down_cell_opt = DotApp_GetCellOptions(Dot_DotApp, down_cell->id);

	if (down_cell_opt->is_empty == true)
	{
		Cell air = {0};
		CellChunk_SetCell(cc_back, x, y + 1, *cell);
		CellChunk_SetCell(cc_back, x, y, air);
		return 0;
	}

	Cell* const left_cell = CellChunk_GetCell(cc_back, x - 1, y + 1);
	Cell* const right_cell = CellChunk_GetCell(cc_back, x + 1, y + 1);
	...
	bool const can_go_right = x < cc_back->width - 1 && right_cell_opt->is_empty == true;
	...
	if (left == true && can_go_left == true)
	{
		// go_left:
		Cell air = {0};
		CellChunk_SetCell(cc_back, x - 1, y + 1, *cell);
		CellChunk_SetCell(cc_back, x, y, air);
		return 0;
	}
